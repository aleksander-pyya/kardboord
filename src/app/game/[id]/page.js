...
export default function GameDetailPage() {
  ...
  // Mängude andmebaas (asenda vajadusel oma API loogikaga)
  const demoGames = [
  ...
  // Leiame õige mängu URL-is oleva ID järgi
  const game = demoGames.find(g => String(g.id) === String(id)) || demoGames[0];
  ...
  return (
    <main className="min-h-screen bg-[#050505] text-white font-sans tracking-tight flex flex-col">
      <div className="max-w-[1400px] mx-auto p-6 md:p-20 flex-grow w-full">
        
        {/* BACK BUTTON */}
        <button onClick={() => router.back()} className="flex items-center gap-2 text-white/30 hover:text-orange-600 transition-all mb-12 uppercase text-[10px] font-black tracking-[0.4em]">
          <ArrowLeft size={16} strokeWidth={3} /> Return to Vault
        </button>

        <div className="grid lg:grid-cols-[450px_1fr] gap-12 md:gap-24 mb-40">
          ...
          <div className="space-y-8">
            ...
            <div className="bg-white/5 p-8 border-l-4 border-orange-600 space-y-6">
              ...
              <a href="#" className="flex items-center justify-between p-4 bg-orange-600 hover:bg-orange-700 transition-colors text-black group">
          ...
          <div className="flex flex-col">
            ...
            <h1 className="text-5xl md:text-9xl font-black italic uppercase tracking-tighter mb-8 leading-[0.85]">{game.name}</h1>
            
            <p className="text-white/50 text-lg md:text-2xl leading-tight mb-12 italic border-l border-white/10 pl-8 max-w-2xl">
              {game.desc}
            </p>
              ...
              <div className={`grid grid-cols-1 sm:grid-cols-2 gap-1 transition-all duration-700 ${alreadyInLibrary ? 'opacity-100' : 'opacity-5 pointer-events-none'}`}>
                <div className="bg-white/5 p-6 border border-white/5">
                  ...
                  <div className="flex gap-3">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <Star key={star} size={24} className="cursor-pointer" fill={star <= userRating ? "#f97316" : "none"} stroke={star <= userRating ? "#f97316" : "white"} strokeWidth={2} onClick={() => { setUserRating(star); updateGameInLibrary({ rating: star }); }} />
                    ))}
                  </div>
                </div>
        ...
        {/* THE FOOTER */}
        <footer className="pt-20 border-t border-white/10 flex flex-col md:flex-row justify-between items-start md:items-center gap-8 pb-20">
          <div>
