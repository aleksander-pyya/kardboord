  ...
  return (
    <main className="min-h-screen bg-[#050505] text-white p-4 md:p-12 font-sans tracking-tight pb-20">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col items-center justify-center pt-10 pb-12 md:pt-24 md:pb-20 relative">
          ...
          <Link href="/" className="transition-all duration-500 block mb-6 md:mb-10">
            <img 
              src="/logo.png" 
              alt="Kardboord Logo" 
              className="h-20 md:h-56 w-auto object-contain drop-shadow-[0_0_25px_rgba(234,88,12,0.3)] md:drop-shadow-[0_0_45px_rgba(234,88,12,0.45)]" 
            />
          </Link>
          
          <div className="flex items-center gap-3 md:gap-6 opacity-40">
            <div className="h-[1px] w-4 md:w-12 bg-white/20"></div>
            <p className="text-[8px] md:text-[11px] font-black uppercase tracking-[0.4em] md:tracking-[0.6em] text-orange-500 italic">
              Digital Vault
            </p>
            <div className="h-[1px] w-4 md:w-12 bg-white/20"></div>
          </div>
        ...
        {/* --- SEARCH: MOBIILIS KITSAM --- */}
        <div className="max-w-xl mx-auto mb-16 md:mb-32">
          <form onSubmit={handleSearch} className="relative">
            ...
            <button type="submit" className="absolute right-2 md:right-4 top-1/2 -translate-y-1/2 bg-orange-600 text-white px-4 py-2 md:px-8 md:py-3.5 rounded-xl md:rounded-2xl font-black uppercase text-[9px] md:text-xs tracking-widest">
              {loading ? <Loader2 className="animate-spin" size={14} /> : "Otsi"}
            </button>
          </form>
        </div>

        {/* --- SEKTSIOONID: MOBIILIS 2 VEERGU --- */}
        {[
